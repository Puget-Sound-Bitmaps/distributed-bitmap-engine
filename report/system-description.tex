...
\begin{figure}
    ...
    \caption[Visualization of Vector Reallocation]{%
        ...
        that vectors must be reallocated. The vectors that must be reallocated
...
vector in the worst case \cite{kleppman2017}. While hash-mod-n requires
\(\func{O}{K}\) remappings, where \(K\) is the number of keys, consistent
hashing only requires \(\func{O}{K/n}\) remappings. \cite{karger1997}
%
\subsection{Queries}
...
order the vectors are \code{OR}ed together within the subquery. The \(swap\)
